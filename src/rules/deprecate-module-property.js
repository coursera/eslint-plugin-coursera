...
module.exports = {
    ...
    create: function(context) {
        ...
        const deprecatedModules = deprecationList
            .reduce((memo, object) => {
        ...
        function checkObjectPatternRequires(node, deprecatedPath) {
            const deprecatedProperties = deprecatedModules[deprecatedPath].map(d => d.property);
        ...
        function getMessage(moduleName, moduleProperty) {
          return deprecatedModules[moduleName]
            .find(d => d.property === moduleProperty)
            .message;
        }
        ...
        function checkAfterCollection() {
            Object.keys(memberObjects)
                ...
                .map(memberObjectKey => {
                    ...
                    const deprecatedProperties = deprecatedModules[moduleName]
                      .map(d => d.property);

                    return memberObjectInstances
                        .map(instance => ({
                            moduleName,
                            instance,
                ...
                .forEach(({ moduleName, instance }) => {
                    const node = context.getSourceCode().getNodeByRangeIndex(instance.index);
                    context.report(
                        node,
